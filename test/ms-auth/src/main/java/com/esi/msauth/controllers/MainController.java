...
public class MainController {
    ...
    private String insert(Inscrits inscrits,Mahrem mahrem,Adresse adresse){
        if(inscrits.getEnum() == null){
            ...
            a = addNewAdresse(null,"sidi bel abbes","sidi brahim");
    ...
    private String tirage(Model model){
        ...
        for (int i=0;i<nmbrplace;i++){
            ...
            if (i<candidats.size()) {
                if(candidats.get(i).getSexe().equals("femme")) {
                    if (i < nmbrplace - 1){
                        nmbrplace = nmbrplace - 1;
                        pelerin.add(candidats.get(i));
                    }
                }
                else  {
                     pelerin.add(candidats.get(i));

                     userRepository.save(new User(null,candidats.get(i).getNom()+i, encoder.encode(candidats.get(i).getPrenom()), true,
                             candidats.get(i).getNom(),candidats.get(i).getPrenom(),candidats.get(i).
                             getDaten(),candidats.get(i).getSexe(),candidats.get(i).getAdresse().getWilaya(),roles));
                }
